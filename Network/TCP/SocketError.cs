...
namespace P2P.TcpConnection {
	public class SocketErrorCode {
		...
		static SocketErrorCode() {
			Array ecs = Enum.GetValues(typeof(ErrorCode));

			errorDictionary = new Dictionary<ErrorCode, string>(ecs.Length);
			for (int i = 0; i < ecs.Length; i++) {
				errorDictionary.Add((ErrorCode)ecs.GetValue(i), errorDiscription[i]);
			}

			errorDetailDictionary = new Dictionary<ErrorCode, string>(ecs.Length);
			for (int i = 0; i < ecs.Length; i++) {
				errorDetailDictionary.Add((ErrorCode)ecs.GetValue(i), errorDetailDiscription[i]);
			}
		}


		/// <summary>
		/// SocketException.ErrorCodeをEnumで表したもの
		/// </summary>
		public enum ErrorCode {
			...
			ERROR_CODE_UNKNOWN = int.MaxValue,//値は適当
		}

		public static string GetErrorDiscription(ErrorCode ec) {
			return errorDictionary[ec];
		}

		private static Dictionary<ErrorCode, string> errorDictionary;
		private static string[] errorDiscription = new string[]{
			...
			"TcpConnection operation on nonsocket.",
			"Destination address required.",
			"Message too long.",
			"Protocol wrong type for socket.",
			"Bad protocol option.",
			"Protocol not supported.",
			"TcpConnection type not supported.",
			...
			"TcpConnection is already connected.",
		...
		public static string GetErrorDetailDiscription(ErrorCode ec) {
			return errorDetailDictionary[ec];
		}

		private static Dictionary<ErrorCode, string> errorDetailDictionary;
		private static string[] errorDetailDiscription = new string[]{
